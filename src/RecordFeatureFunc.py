...
def calculate(flow_record, host_model):
    ...
    path_set = host_model['record_box']['path_set']
    ...
    if flow_record['path'] in path_set:
        ...
        flow_record['specialSymbol_prop'] = 'Default'
        flow_record['enumeration'] = 'Default'
        flow_record['variable_composition'] = 'Default'
        flow_record['variable_order'] = 'Default'
        flow_record['value_length_prop'] = 'Default'
        flow_record['value_distribution1'] = 'Default'
        flow_record['value_distribution2'] = 'Default'
...
def _cal_path_prop(flow_record, host_path_element_count):
    ...
    for i in range(path_length - 1):
        ...
        probability += prop_i
    # eliminate the HEAD and END segment
    probability /= (path_length - 2)
...
def _cal_special_symbol_prop(flow_record, host_value_special_symbol_prop):
    ...
    if path not in host_value_special_symbol_prop:
        return 'Default'
    else:
        min_prop = 1
        variable_dict = host_value_special_symbol_prop[path]
...
def _cal_enumeration(flow_record, host_variable_enumeration):
    ...
    variable_value_dict = flow_record['variable_value_dict']
    path = flow_record['path']
    if path not in host_variable_enumeration:
        return 'Default'
    else:
        variable_dict = host_variable_enumeration[path]
        for variable, value in variable_value_dict.items():
...
def _cal_variable_composition(flow_record, host_variable_composition_pool):
    ...
    """
    path = flow_record['path']
    if path not in host_variable_composition_pool:
        return 'Default'
    else:
        record_variable_composition = frozenset(flow_record['variable_value_dict'].keys())
        variable_composition_pool = host_variable_composition_pool[path]
...
def _cal_variable_order(flow_record, host_variable_order_rule):
    ...
    """
    path = flow_record['path']
    if path not in host_variable_order_rule:
        return 'Default'
    else:
        ...
        if para:
            ...
            variable_order_rule = host_variable_order_rule[path]
...
def _cal_value_length_prop(flow_record, host_value_length_distribution):
    ...
    """
    path = flow_record['path']
    if path not in host_value_length_distribution:
        return 'Default'
    else:
        variable_dict = host_value_length_distribution[path]
        variable_value_dict = flow_record['variable_value_dict']
...
def _cal_value_distribution1(flow_record, host_value_distribution1):
    ...
    """
    path = flow_record['path']
    if path not in host_value_distribution1:
        return 'Default'
    else:
        variable_dict = host_value_distribution1[path]
        variable_value_dict = flow_record['variable_value_dict']
...
def _cal_value_distribution2(flow_record, host_value_distribution2):
    ...
    """
    path = flow_record['path']
    if path not in host_value_distribution2:
        return 'Default'
    else:
        variable_dict = host_value_distribution2[path]
        variable_value_dict = flow_record['variable_value_dict']
    ...
    with open(flow_address, 'rb') as infile, open(flow_feature_stored_address, 'wb') as outfile:
        for line in infile:
            ...
            if len(record) == 13 and record[_host] != '':
                ...
                # calculate_special_symbol_prop(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['specialSymbol_prop'])

                # calculate_enumeration(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['enumeration'])

                # calculate_variable_composition(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['variable_composition'])

                # calculate_variable_order(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['variable_order'])

                # calculate_value_length_prop(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['value_length_prop'])

                # calculate_value_distribution1(current_record, current_model)
                # outfile.write('%s\n' % current_record['url'])
                # outfile.write('\t%s\n' % current_record['value_distribution1'])

                calculate_value_distribution2(current_record, current_model)
                outfile.write('%s\n' % current_record['url'])
